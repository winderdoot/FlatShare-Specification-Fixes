...
\begin{itemize}
    ...
    \item \textbf{Właściciel} publikuje ogłoszenia w spójnej formie, kontroluje dostępność i może akceptować rezerwacje.
...
\textbf{Opis:} Rezerwacja odzwierciedla proces biznesowy wynajmu: od złożenia prośby przez Lokatora, przez akceptację Właściciela , aż po płatność i potwierdzenie. Diagram obejmuje również zakończenia nietypowe: odrzucenie, wygaśnięcie, błąd płatności \textcolor{red}{oraz wymuszone anulowanie administracyjne}.
\begin{itemize}
...
\textbf{Opis:} Proces rezerwacji jest kluczowym procesem biznesowym Lokatora. Diagram uwzględnia standardowy przepływ z akceptacją Właściciela, oraz alternatywy: odrzucenie, wygaśnięcie, błąd płatności i kolizję dostępności.
\begin{itemize}
  ...
  \item \textbf{Alternatywy:} \texttt{REJECTED} (odrzucenie), \texttt{EXPIRED} (brak reakcji), \texttt{PAYMENT\_FAILED} (możliwość ponowienia), kolizja dostępności (anulowanie procesu).
...
\subsection{Sytuacje wyścigu}
Dodatkowo, w procesach zależnych od dostępności zasobu (np. rezerwacja terminu)
...
\begin{itemize}
    ...
    \item Adres URL: \texttt{/api/v1/users}
...
\begin{lstlisting}[style=http,caption={Przykładowe żądanie rejestracji}]
POST /api/v1/users
Content-Type: application/json
Accept: application/json
\end{lstlisting}

\begin{lstlisting}[style=json,caption={Ciało żądania rejestracji}]
{
  "firstName": "Jan",
  "lastName": "Kowalski",
...
Pole \textit{role} przyjmuje następujące wartości:
\begin{itemize}
  \item \texttt{"LANDLORD"} - Właściciel
  \item \texttt{"TENANT"} - Lokator
\end{itemize}
...
\begin{lstlisting}[style=http,caption={Odpowiedź HTTP — nieudana rejestracja}]
400 Bad Request
\end{lstlisting}

%============================================%
\section{Pobieranie danych użytkownika}

\subsection{Opis endpointu}

Służy do pobierania podstawowych informacji o użytkowniku - odzwierciedla zawartość nagłówka \textit{Location} w odpowiedzi endpointu rejestracji. W url nalezy wstawić id użytkownika.

\begin{itemize}
    \item Metoda HTTP: \texttt{GET}
    \item Adres URL: \texttt{/api/v1/users/1234-1234-1234-1234}
    \item Typ treści: \texttt{application/json}
\end{itemize}
...
autoryzacji żądań kierowanych do endpointów chronionych interfejsu API. Aby logowanie było restful i polegało wyłącznie na tworzeniu/dostępie do zasobów, reprezentujemy logowanie jako endpoint tworzący nową sesję. 
...
\begin{itemize}
    \item Metoda HTTP: \texttt{POST}
    \item Adres URL: \texttt{/api/v1/sessions}
    \item Typ treści: \texttt{application/json}
\end{itemize}
...
użytkownika w systemie (utworzenia sesji)
...
W przypadku poprawnego uwierzytelnienia użytkownika system zwraca informację o utworzeniu sesji reprezentowanej przez token JWT,
...
Jednakże, w praktyce potrzebny jest jeszcze sposób na odświeżenie sesji (tokenu) użytkownika, dlatego prezentujemy następujący endpoint:

%=============================================%

%============================================%
\section{Odświeżanie sesji}

\subsection{Opis endpointu}

\begin{itemize}
    \item Metoda HTTP: \texttt{PATCH}
    \item Adres URL: \texttt{/api/v1/sessions}
    \item Typ treści: \texttt{application/json}
...
W przypadku poprawnego uwierzytelnienia użytkownika system zwraca informację o odświeżeniu sesji reprezentowanej przez token JWT,
który powinien zostać zapisany po stronie klienta i dołączany do kolejnych żądań.
...
\begin{lstlisting}[style=http,caption={Odpowiedź HTTP — błędna autoryzacja}]
401 Unauthorized
\end{lstlisting}
%===================================%
\section{Pobieranie informacji o sesji}

\subsection{Opis endpointu}
Endpoint ten jest dla formalizmu, aby nagłówek \textit{Location}
przy tworzeniu sesji (Logowanie) miał sens.
...
%===========================================================%
\section{Zarządzanie preferencjami}
...
\subsection{Opis endpointu - ustawianie preferencji}
...
\subsection{Opis endpointu - pobieranie preferencji}
...
\section{Wyszukiwanie pasujących mieszkań}
...
\begin{itemize}
    ...
    \item \texttt{city} – miasto.
    \item \texttt{district} – dzielnica miasta.
    \item \texttt{minPrice} – minimalna cena.
    \item \texttt{maxPrice} – maksymalna cena.
    \item \texttt{petsAllowed} – zezwolenie na mieszkanie ze zwierzętami.
    \item \texttt{nonSmokingOnly} – wymaganie aby wszyscy lokatorzy byli niepalący.
    \item \texttt{closeToShops} – liczba elementów na stronie.
    \item \texttt{profile} – jedna z wartości: "student", ... (otwarte na rozszerzenie)
    \item \texttt{minArea} – minimalna powierzchnia pokoju [$m^2$].
    \item \texttt{maxArea} – maksymalna powierzchnia pokoju [$m^2$].
    \item \texttt{startDate} - początek okresu wynajmu
\end{itemize}
...
(\texttt{Draft/UnderReview/Active/Hidden/Archived}) ma bezpośrednie odzwierciedlenie w API.

\subsection{Przykładowe żąd anie}

\begin{lstlisting}[style=http,caption={Żądanie HTTP — wystawienie szczegółów ogłoszenia}]
...
\begin{lstlisting}[style=http,caption={Żądanie HTTP — pobranie szczegółów ogłoszenia}]
GET /api/v1/listings
Authorization: Bearer <JWT>
Content-Type: application/json
Accept: application/json
...
\begin{lstlisting}[style=json,caption={Ciało żądania — pobranie szczegółów ogłoszenia}]
{
  "title": "Pokój przy metrze",
  "description": "Najlepszy pokój, nawet karaluchów nie ma",
  "price": 1200.00,
...
\subsection{Uwagi projektowe}
Walidacja danych wejściowych realizowana
...
\noindent\textbf{Uwagi komunikacyjne:} pole \texttt{clientMessageId} pozwala klientowi na deduplikację w przypadku ponowień.
